...
export function EpoxiDayDetailModal({data,detalhes,faltas,observacoes,turno,onClose}:{
  ...
  const producao = detalhes.filter(r=>r.data===data && r.setor==='EPOXI' && r.linha==='EPO');
  const faltasDia = faltas.filter(r=>r.data===data && r.setor==='EPOXI' && r.linha==='EPO' && (turno==='Todos'||r.turno===turno));
  const obsDia = observacoes.filter(r=>r.data===data && r.setor==='EPOXI' && r.linha==='EPO');
